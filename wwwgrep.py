...
def get_raw_response(url_arg,search_string=None):
	...
	if search_scripts:
	 	# OBTAIN ALL SCRIPTS ON THE PAGE
	 	if not search_java_offpage:
	 		parser = 'html5lib'
	 		soup = BeautifulSoup(response.text, parser)
	 		...
	 		for source in script_src:
	 			# SOME RECURSION HERE
	 			if str(source['src']).upper().strip() not in js_list:
	 				js_list.append(str(source['src']).upper().strip())
	 				soup = BeautifulSoup(get_url(source['src']).text, parser)
	 				if_append(soup.find_all(text=True),search_string, url_arg, str(separator))
